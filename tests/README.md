...
| [`test_fit_hooks.py`](test_fit_hooks.py) | `fit(optimizer=, after_epoch_callbacks=)` the Keras-shaped validation/verbose options and the shipped `tramdag.callbacks` — stop, lists, `RestoreBest`, `PerNodePlateau`, a torch scheduler; the guard that it still lands on the MLE |
